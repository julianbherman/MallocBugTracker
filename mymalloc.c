...
void *mymalloc(int size){
	...
	while ( *(int*)p )
	{
		...
		{
			void* final_ptr = allocateChunk(p, padded_size, curr_chunk_size);
			//printf("%p", final_ptr);
			return final_ptr;
		}       
...
/* returns 1 if ptr was generated by mymalloc(), 0 otherwise */
static int ptrValid(void *ptr){
	double* p = memory;
	...
	while ( *(int*)p )
	{
		...
		{
			return 1;	// ptr is valid!
		}
		p += HEADERSIZE + abs(*(int*)p);	// increment p
	}
	return 0;	// ptr is NOT valid!
}
...
void myfree(void *ptr){
		...
		// prev_chunk and next_chunk are both free and IN BOUNDS OF MEM
	{
		...
		*prev_hdr_ptr = -jump_amount - *curr_hdr_ptr + 
		...
		if ( *next_next_hdr_ptr )// next_next_hdr_ptr is IN BOUNDS OF MEM
		{
